...
# -*- coding: utf-8 -*-
"""
Binance USDT Î¨¥Í∏∞Ì?? Ï?†Î¨º ‚?? BB Ï?§Ì?¥Ï¶? + Î∞¥Î?? Îè?Ì?? + RSI Í∏∞Ï?∏Í∏∞ + ATR SL (15Î∂?Î¥?).
...
# OHLC Ì??Ï?§Ì?†Î¶¨ (15Î∂? Ì??Ï†?Î¥?Îß? append)
...
def _sl_close_title(pnl_pct: float) -> str:
    # Ï??Ïù¥ÏΩ?Îß? Î¥êÎè? Íµ¨Î∂?: ??¢ Ïùµ(Ï¥?Î°ù) / ??¥ Ï?ê(Îπ®Í∞?) / ‚??Ô∏è Î≥∏Ï†?
    if pnl_pct > 0:
        return "??¢ ÏùµÏ†? Ï≤≠Ï?∞ (Îß?ÌÅ¨ SL)"
    if pnl_pct < 0:
        return "??¥ Ï?êÏ†? Ï≤≠Ï?∞ (Îß?ÌÅ¨ SL)"
    return "‚??Ô∏è Î≥∏Ï†? Ï≤≠Ï?∞ (Îß?ÌÅ¨ SL)"
...
def update_trailing_sl(
    ...
    """
    15Î∂? Î¥? Îß?Í∞ê Ï?? ATR√?Î∞∞Ï??Î°? Ì?∏Î†?Ïùº.
...
def time_exit_close(symbol_upper: str) -> None:
    """TIME_EXIT_BARS Í≤ΩÍ≥º + Ì?∏Î†?Ïùº ÎØ∏Ì??Ï?± Ï?? Ï??Ï?•Í∞? Ï≤≠Ï?∞."""
    with state_lock:
    ...
    with state_lock:
        ...
        save_state(st)
    tg.send_message(
        f"‚è∞ Ï??Í∞?Ï¥?Í≥º Ï≤≠Ï?∞: #{symbol_upper}\n"
        f"Ìè¨Ïß?Ï?? : {direction.upper()}\n"
        f"Ïß?Ï??Í∞?: {_fmt(entry)}\n"
        f"Ï≤≠Ï?∞Í∞?: {_fmt(mk)}\n"
        f"Ï?êÏùµ: {pnl_pct:+.2f}%\n"
...
def execute_entry(symbol_upper: str, pe: Dict[str, Any]) -> None:
    ...
    if lp > HIGH_VOL_MAX_SL_PCT:
        tg.send_message(
            f"‚?? Ïß?Ï?? SKIP (SL Ï¥?Í≥º): #{symbol_upper}\n"
            f"SLÍ±∞Î¶¨: {lp*100:.2f}%\n"
            f"{_binance_footer(symbol_upper)}"
        )
        pending_entry.pop(symbol_upper, None)
    ...
    if not res:
        tg.send_message(f"‚ù? Ïß?Ï?? Ï?§Ì?® #{symbol_upper}\n{_binance_footer(symbol_upper)}")
        pending_entry.pop(symbol_upper, None)
        return
    ...
    tg.send_message(
        f"{side_ico} Ïß?Ï?? {direction.upper()}\n"
        f"#{symbol_upper}\n"
        f"Ïß?Ï??Í∞?: {_fmt(entry)}\n"
        f"Ï¥?Í∏∞ SL: {_fmt(sl0)}\n"
        f"ATR(Ï?†Ì?∏Î¥?): {_fmt(signal_atr)}\n"
        f"{_binance_footer(symbol_upper)}"
    )
    if high_vol:
        tg.send_message(
            f"‚?° Í≥†Î≥?Îè?Ï?± Ïß?Ï?? (Ï?¨Ïù¥Ï¶? Ï∂?Ï??): #{symbol_upper} {direction.upper()}\n"
            f"SLÍ±∞Î¶¨: {lp*100:.2f}% Ï?¨Ïù¥Ï¶?: {HIGH_VOL_POSITION_SIZE_PCT*100:.1f}%\n"
            f"{_binance_footer(symbol_upper)}"
        )
...
def process_kline(symbol_lower: str, k: Dict[str, Any]) -> None:
    ...
    if in_pos:
        ...
        # Ì?∏Î†?Ïùº Ì??Ï?±Ì??: 15Î∂? Ì??Ï†?Î¥? Ï¢?Í∞?Îß? Ï?¨Ï?© (Ï?†Ì?∏Î¥? ATR √? Î∞∞Ï??)
            ...
            if act:
                ...
                with state_lock:
                    if symbol_upper in active_positions:
                        ...
                        save_state(st)
                tg.send_message(
                    f"??Ø Ì?∏Î†?ÏùºÎßÅ Ì??Ï?±Ì??: #{symbol_upper} {direction.upper()}\n"
                    f"Ïß?Ï??Í∞?: {_fmt(entry)}\n"
                    f"Ì??Ï?±Ì??Í∞?: {_fmt(c)}\n"
                    f"{_binance_footer(symbol_upper)}"
                )
        ...
        if trail_active:
            ...
            if chg:
                tg.send_message(
                    f"??? SL Í∞±Ï?† #{symbol_upper}\n"
                    f"Ïù¥Ï†? SL: {_fmt(old_sl)}\n"
                    f"Ï?? SL: {_fmt(new_sl)}\n"
                    f"{_binance_footer(symbol_upper)}"
                )
        else:
...
def mark_monitor_loop() -> None:
    while True:
        ...
        try:
            ...
            try:
                tg.send_message(f"‚ù? Îß?ÌÅ¨ Í∞êÏ?? Ï?§Î•?\n{e!s}")
            except Exception:
                pass
...
def cmd_status() -> None:
    ...
    tg.send_message(
        ...
        f"Ï?†Í∑?Ïß?Ï??: {st}\n"
        f"Ï??Í≥†: {eq:.2f} USDT\n"
        f"Ìè¨Ïß?Ï??: {n}/{MAX_CONCURRENT_POSITIONS}\n"
        f"Í∞êÏ??: {len(tracked_symbols)} Ï?¨Î≥º"
    )
...
def cmd_stop() -> None:
    global bot_active
    bot_active = False
    tg.send_message("‚è∏Ô∏è Ï?†Í∑? Ïß?Ï?? Ï§?Ïß?")


def cmd_restart() -> None:
    global bot_active
    bot_active = True
    tg.send_message("‚?∂Ô∏è Ï?†Í∑? Ïß?Ï?? Ï?¨Í∞?")
...
def universe_refresh_loop() -> None:
    ...
    while True:
        ...
        try:
            ...
            tg.send_message(
                f"??? Ï?†Î??Î≤?Ï?§ Í∞±Ï?† ({SYMBOL_REFRESH_INTERVAL}s)\nÎ??Ï?Å {len(merged)}Í∞?"
            )
        except Exception as e:
            logger.exception("universe_refresh: %s", e)
            try:
                tg.send_message(f"‚ù? Ï?†Î??Î≤?Ï?§ Í∞±Ï?† Ï?§Î•?\n{e!s}")
            except Exception:
                pass
...
def main() -> None:
    ...
    uni = select_universe()
    if not uni:
        tg.send_message("‚ù? Ï?†Î??Î≤?Ï?§ Ï°∞Ì?? Ï?§Ì?®")
        remove_lock()
        return
    ...
    tg.send_message(
        f"??? BB Ï?§Ì?¥Ï¶? Î¥? Ï??Ï??\n"
        f"15Î∂?Î¥? ¬∑ Î†?Î≤? {DEFAULT_LEVERAGE}x ¬∑ Ïß?Ï?? {POSITION_RISK_PCT*100:.0f}% "
        f"(Í≥†Î≥?Îè? {HIGH_VOL_POSITION_SIZE_PCT*100:.1f}%) ¬∑ Ïµ?Î?? {MAX_CONCURRENT_POSITIONS}Ìè¨Ïß?Ï??\n"
        f"SL: ATR{ATR_PERIOD}√?{ATR_MULTIPLIER} ¬∑ SLÏ∫° ÏùºÎ∞?‚?§{MAX_SL_PCT*100:.0f}% Í≥†Î≥?Îè?‚?§{HIGH_VOL_MAX_SL_PCT*100:.0f}%\n"
        f"Ì?∏Î†?Ïùº Ì??Ï?±: 15Î∂? Ï¢?Í∞?(Ïß?Ï??Í∞? ¬± Ï?†Ì?∏ATR√?{TRAIL_ACTIVATE_MULTIPLIER}) ¬∑ "
        f"ÎØ∏Ì??Ï?± {TIME_EXIT_BARS}Î¥? Ï?? Ï??Í∞?Ï¥?Í≥º Ï≤≠Ï?∞\n"
        f"Í∞êÏ?? Ï?¨Î≥º: {len(tracked_symbols)}"
    )
...
if __name__ == "__main__":
    try:
        ...
        try:
            tg.send_message(f"??• Î¥? Ïπ?Î™?Ï†Å Ï?§Î•?\n{e!s}")
        except Exception:
            pass
        remove_lock()
        sys.exit(1)
